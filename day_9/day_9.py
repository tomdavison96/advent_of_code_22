...
def move_head(current_x: int, current_y: int, direction: str) -> Tuple[int, int]:
    """Move the head of the rope in the given direction by 1 unit and return the new x, y coordinates"""
    if direction == 'L':
        current_x -= 1
    elif direction == 'R':
        current_x += 1
        ...
        raise ValueError(f'Unexpected direction provided: {direction}')
...
def move_tail(tail_x: int, tail_y: int, head_x: int, head_y: int) -> Tuple[int, int]:
    ...
    elif (abs(x_diff) == 1 and abs(y_diff) == 2) or (abs(y_diff) == 1 and abs(x_diff) == 2):
        tail_x += math.copysign(1, x_diff)
        tail_y += math.copysign(1, y_diff)
...
for instruction in instructions:
    ...
    for i in range(distance):
        ...
        head_locations += [(x, y)]
        tail_x, tail_y = move_tail(tail_x, tail_y, x, y)
        tail_locations += [(tail_x, tail_y)]
...
for instruction in instructions:
    ...
    for i in range(distance):
        ...
        for knot in range(1, rope_length):  # Iterate through other parts of the rope and treat each one as a 'tail'
            head_part_x, head_part_y = rope_positions[knot - 1]
            tail_part_x, tail_part_y = rope_positions.pop(knot)
            tail_part_x, tail_part_y = move_tail(tail_part_x, tail_part_y, head_part_x, head_part_y)
            rope_positions.insert(knot, (tail_part_x, tail_part_y))
            assert len(rope_positions) == 10
